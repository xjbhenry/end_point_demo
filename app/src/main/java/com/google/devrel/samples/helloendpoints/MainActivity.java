...
import com.appspot.brave_monitor_820.helloworld.Helloworld.Greetings.GetGreeting;
import com.appspot.brave_monitor_820.helloworld.Helloworld.Greetings.Multiply;
...
import com.appspot.brave_monitor_820.helloworld.Helloworld.Greetings.Authed;
...
public class MainActivity extends Activity {
    ...
    public void onClickListGreetings(View view) {
        View rootView = view.getRootView();
        TextView greetingIdInputTV = (TextView)rootView.findViewById(R.id.greeting_id_edit_text);
        if (greetingIdInputTV.getText()==null ||
                Strings.isNullOrEmpty(greetingIdInputTV.getText().toString())) {
        ...
        AsyncTask<Void, Void, HelloGreetingCollection> getAndDisplayGreeting =
                new AsyncTask<Void, Void, HelloGreetingCollection> () {
                    ...
                    protected HelloGreetingCollection doInBackground(Void... unused) {
                        ...
                        try {
                            HelloGreetingCollection greetings = apiServiceHandle.greetings().listGreeting().execute();
                            return greetings;
                        } catch (IOException e) {
                            Log.e(LOG_TAG, "Exception during API call", e);
                        }
                    ...
                    @Override
                    protected void onPostExecute(HelloGreetingCollection greetings) {
                        if (greetings!=null) {
                            displayGreetingList(greetings.getItems());
                        } else {
                            Log.e(LOG_TAG, "No greetings were returned by the API.");
                        }
                    }
    ...
    private void displayGreetingList(List<HelloGreeting> greetings) {
        String msg;
        if (greetings==null || greetings.size() < 1) {
            ...
            //HelloGreeting[] greeting_array = greetings.toArray(new HelloGreeting[java.lang.Math.min(greetings.size(),10)]);
            //List<HelloGreeting> greetingsList = Arrays.asList();

            int list_number = java.lang.Math.min(greetings.size(), 10);
            HelloGreeting[] greeting_array = new HelloGreeting[10];
            for (int i=0; i<10; i++) greeting_array[i] = greetings.get(i);
            mListAdapter.replaceData(greeting_array);
